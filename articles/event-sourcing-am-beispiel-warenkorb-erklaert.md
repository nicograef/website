...
- `POST /warenkorb { "produktId": 123, "menge": 2 }`
  - um ein Produkt hinzuzufügen (CREATE)
- `PATCH /warenkorb/produkt/123 { "menge": 4 }`
  - um die Menge eines Produkts zu ändern (UPDATE)
- `DELETE /warenkorb/produkt/123`
  - um ein Produkt zu entfernen (DELETE)
- `GET /warenkorb`
  - um den aktuellen Zustand des Warenkorbs abzurufen (READ)
...
- `POST /warenkorb/produkt-hinzufuegen { "produktId": 123, "menge": 2 }`
  - um ein Produkt hinzuzufügen (schreibt ein "produkt-hinzugefuegt" Event)
- `POST /warenkorb/produkt-entfernen { "produktId": 123 }`
  - um ein Produkt zu entfernen (schreibt ein "produkt-entfernt" Event)
- `POST /warenkorb/menge-aendern { "produktId": 123, "neueMenge": 4 }`
  - um die Menge eines Produkts zu ändern (schreibt ein "menge-geaendert" Event)
- `GET /warenkorb`
  - um den aktuellen Zustand des Warenkorbs abzurufen (liest alle Events und rekonstruiert den Zustand)
...
1. Wir lesen alle Events für Benutzer 1 aus der Tabelle:
...
2. Wir rekonstruieren den Warenkorb:
   - Nach Event 1: Warenkorb enthält Produkt 123 mit Menge 2
   - Nach Event 3: Warenkorb enthält Produkt 123 mit Menge 1
   - Nach Event 4: Warenkorb enthält Produkt 123 mit Menge 1 und Produkt 456 mit Menge 2
   - Nach Event 5: Warenkorb enthält nur noch Produkt 456 mit Menge 2
   - Nach Event 7: Warenkorb enthält nur noch Produkt 456 mit Menge 3
3. Wir geben den aktuellen Zustand des Warenkorbs zurück
