...
| Code examples | Fenced code blocks with language tag (```sql, ```json, etc.) — only when code conveys insight that prose cannot |
...
- No glossary.
- No diagrams, images, or image placeholders — the user adds these manually
  if needed after the article is written.
...
- The self-review in Step 5 is the primary quality gate for article content.
